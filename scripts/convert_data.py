...
with open(src_file, 'r') as myfile:
    ...
    for row in src_data:
        ...
        if len(graph_values.split(" ")) > 5:
            ...
            weight_muscle = float(weight_muscle) if weight_muscle.upper() != "X" else None
...
cur = conn.cursor()
cur.executemany("""INSERT INTO bp_and_weight(date, time, sys, dia, bp, weight_total, weight_fat, weight_muscle, comment) 
                VALUES (
                    %(date)s,
                    %(time)s,
                    %(bp_sys)s,
...
cur.executemany("""INSERT INTO bp_and_weight_comments(date, comment)
                VALUES (
                    %(date)s,
                    %(comment)s
                )
